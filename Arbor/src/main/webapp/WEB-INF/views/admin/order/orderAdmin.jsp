			...
			<form method="post" id="j_orderListFrm" action="orderAdmin">
				...
				<ul>
					...
					<c:forEach var="orderVo" items="${list }">
						...
						<li>
							<c:choose>
							<c:when test="${orderVo.productcnt==1 }">${orderVo.pname }</c:when>
							<c:otherwise>${orderVo.pname } 외 ${orderVo.productcnt-1 }건</c:otherwise>
							</c:choose>
				...
				<c:forEach var="p" begin="${pageVO.startPageNum }" step="1" end="${pageVO.startPageNum + pageVO.onePageNum-1 }">
					<c:if test="${p<=pageVO.totalPage }">
						<c:if test="${p==pageVO.pageNum }">
							<li style="border-bottom:3px solid rgb(191,43,53);"><a href="orderAdmin?pageNum=${p }<c:if test="${pageVO.searchWord != null && pageVO.searchWord != ''}">&searchKey=${pageVO.searchKey }&searchWord=${pageVO.searchWord }</c:if>">${p }</a></li>
						</c:if>
						<c:if test="${p!=pageVO.pageNum }">
							<li><a href="orderAdmin?pageNum=${p }<c:if test="${pageVO.searchWord != null && pageVO.searchWord != ''}">&searchKey=${pageVO.searchKey }&searchWord=${pageVO.searchWord }</c:if>">${p }</a></li>
						</c:if>
					</c:if>
				</c:forEach>
