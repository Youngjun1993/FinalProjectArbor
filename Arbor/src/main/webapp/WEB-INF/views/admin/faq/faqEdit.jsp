...
<div class="w1400_container font_ng clearfix">
	...
	<div class="d_noticeEdit">
		<h2 class="d_h1">FAQ 수정</h2>
		<form method="post" id="faqEditFrm" action="faqEditOk" enctype="multipart/form-data">
			<input type="hidden" name="faqno" value="${faqEdit.faqno }"/>
			<div>
				...
				<div>
					...
					<select name="faqcate" id="d_faqCate" value="${faqEdit.faqcate}">
						...
						<c:if test="${faqEdit.faqcate == '교환반품'}">
							<option value="교환반품" selected>교환반품</option>
						</c:if>
						<c:if test="${faqEdit.faqcate!='교환반품' }">
							<option value="교환반품">교환반품</option>
						</c:if>
						
						<c:if test="${faqEdit.faqcate == '배송문의'}">
				...
				<br/>
				<textarea name="faqcontent" id="d_faqInsert">${faqEdit.faqcontent }</textarea><br/>
				<p class="d_noticeInsertBtn"><input type="button" class="adminMainBtn" id="d_faqEditBtn" value="수정"> <input type="button" class="adminSubBtn" id="d_faqCnlBtn" value="취소"></p>
			</div>
		</form>
	</div>
</div>
